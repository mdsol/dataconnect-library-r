...
*Note:* The `init()` example above assumes that we're working with iMedidata production. If you need to work with the `Innovate` (pre-production) stage for any reason, add an additional `url` parameter to the `init()` function as shown below:

`dc <- init(token = userToken, url = "enodia-gateway-innovate.platform.imedidata.com")`
...
tryCatch({
  result <- dc$studies(search_study_name = "")
  ...
  # Instead of printing, exception can be handled programmatically based on the error code or message.
  print(e)
