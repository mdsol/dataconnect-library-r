...
```{r}
# we recommend store userToken in a seperate file in your working directory
...
Retrieve the study and study environment information Developer information in iMedidata Platform

```{r}

PWBStudy = list(
  PWBStudyId = "studyid",
  PWBPRODEnvID = "studyenviromentid"
)

ExciterStudy = list (
  ExciterStudyId = "studyid",
  ExciterPRODEnvID = "studyenviromentid"
)
```
...
```{r}
#datasetid can retrieved from search datasets()
datasetid = list(
  PWBDerID = "datasetid",
...
```{r}
# Consider choose first few row of a dataset by using %>%head() if the dataset is extremely large 

RAVEdf = RAVEdata$frame %>% 
  head() %>%
  collect()
```
...
If you have limited memory allocation, we recommend working on a limited set of record to reduce development time, and then remove record limit during recurring execution.

If you receive error indicating you are running out of memory, please reach out your R IDE provider to increase memory allocation.

#### Set up: Using dplyr to perform data transformation on materialized data frame

Those functions are not provided by `dataconnect` library, below are just examples how common R librarys can be used with the `dataconnect` library. For detail of how those library can be used, please consult the library providers.
...
# union the dataset together (pivotDF, Importdf) as unionDF
...
# join the dataset together (uninoDF, Customdf) as publishDF
x = Importdf
y = unionDF
publishDF <- full_join(
  x,
...
Set up your project token where the resulting dataset (i.e., publishDF) will be pushed to. Please make sure it is published to the correct study environment and project. It recommended to use dry_publish function to validate the result first before publish the resulting dataset back to Medidata Data Connect.

```{r}
# Project token is generated in Data Connect Transformation Custom Code Project

myProjectToken = "projectoken"
```
