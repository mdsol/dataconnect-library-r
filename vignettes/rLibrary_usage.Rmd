...
```{r eval=FALSE}
# Fetch supported formats from Data Connect.
all_formats <- dc$get_datetime_formats(
  project_token = myProjectToken,
  type = "all"
)

# Filter for date-only or datetime-only values when needed.
date_formats <- dc$get_datetime_formats(
  project_token = myProjectToken,
  type = "date"
)

datetime_only_formats <- dc$get_datetime_formats(
  project_token = myProjectToken,
  type = "datetime"
)

# Example mapping used by dry_publish()/publish().
datetime_formats_list <- list(
  "start_date" = "MM/dd/yy",
  "end_date" = "yyyy-MM-dd",
